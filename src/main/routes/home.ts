...
import { ValidatedForm } from '../app/validatedForm'
import { Amount } from '../model/amount'

export default Router().get('/', (req, res) => {
    res.render('index.njk', {
        form: new ValidatedForm<Invoice>(new Invoice("", "", [new Amount()]))
    })
}).post('/', FormParser.createMiddleware<Invoice>(Invoice.fromJSON), (req, res) => {

    if(res.locals.form.errors.length > 0) {
        res.render('index.njk', {
            form: res.locals.form
        })
    } else {
        //TODO: This request needs to be a client
        const data = {
            ...
            content: {
                services: res.locals.form.model.amountLines.map((line: any) => {
                    return {
                        name: line.desc,
        ...
        let request_test = require('request')
        const options = {
